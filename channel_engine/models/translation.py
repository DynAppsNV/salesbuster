...
class Products(models.Model):
    ...
    def get_translated_data(self):
        ...
        for item in map_list:
            ...
            for rec in src:
                translated_data.append({"key": (item[1].split(',')[-1]) + ' - ' + rec.lang, "Value": rec.value, "Type": "TEXT", "IsPublic": True})
            if src:
                translated_data.append({"key": (item[1].split(',')[-1]) + ' - EN_us', "Value": self.cleanhtml(src[0].source), "Type": "TEXT", "IsPublic": True})
