...
class ce_instance_config(models.TransientModel):
    ...
    def create_ce_instance(self):
        ...
        if exist_instance:
            raise Warning("Instance is already exist")
        else:         
...
class ce_config_settings(models.TransientModel):
    ...
    update_ce_price_interval_number = fields.Integer(string='Channel Update Stock Interval Number',help="Repeat every x.")
        ...
        if self.is_auto_update_products_stock:
            ...
            if cron_exist:
                ...
                if not ce_update_stock_cron_exist:
                    raise Warning('Core settings of Channel are deleted, please upgrade channel engine Connector module to back this settings.')
